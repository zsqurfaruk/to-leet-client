    ...
    <div className="card-body min-h-44">
      <div className="-mt-2 text-warning">
        {university?.title?.length > 35 ? (
          ...
          <h2 className="text-[13px] font-semibold"> {university?.title}</h2>
        )}
      </div>
     ...
     {university?.bedrooms?.eng && (
          <div className="flex gap-1">
            ...
            {!lang ? (
              ...
              <h2><span className="text-[13px]">বেডরুম</span>: {university?.bedrooms?.ban}</h2>
            )}
          </div>
        )}
        ...
        {university?.bedNumber && (
          <div>
            {!lang ? (
              ...
              <h2><span className="text-[13px]">ফাঁকা বেড</span>: {university?.bedNumber?.ban} </h2>
            )}
          </div>
        )}
        {
          university?.bathrooms && <div
          className={
            university?.type?.eng === "Mess-(Male)" ||
            university?.type?.eng === "Mess-(Female)" ||
            university?.type?.eng === "Sublet-(Male)" ||
        ...
        >
          ...
          {!lang ? (
            ...
            <h2><span className="text-[13px]">বাথরুম</span>: {university?.bathrooms?.ban}</h2>
          )}
        </div>
        }
      </div>
        ...
        <div>
          {!lang ? (
            ...
            <h2 className="text-[13px]">
              ...
              {university?.negotiable && <span className="text-[12.8px]">আলোচনা সাপেক্ষে</span> }
            </h2>
          )}
        </div>
      </div>
      ...
      {university?.cityName?.eng && (
          <div className="flex">
            <MdOutlineLocationOn className="text-warning h-4 w-5 -ml-1"/>{!lang ? (
              <h2>
                  {university?.areaName?.eng.length > 17 ? <span> {university?.areaName?.eng.slice(0,17)}</span> :  <span>{university?.areaName?.eng}</span> }
              </h2>
            ) : (
              <h2 className="text-[13px]">
                {" "}
                {university?.areaName?.ban.length > 18 ? <span> {university?.areaName?.ban.slice(0,18)}</span> :  <span>{university?.areaName?.ban}</span> }
              </h2>
            )}
            {!lang ? (
              <h2>
